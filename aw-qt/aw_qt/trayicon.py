...
class TrayIcon(QSystemTrayIcon):
    def __init__(
        ...
        self.setToolTip("GFP ActivityWatch Admin" + (" (testing)" if testing else ""))

        self.manager = manager
        self.testing = testing

        self.root_url = f"http://localhost:{5666 if self.testing else 5800}"
    ...
    def _build_rootmenu(self) -> None:
        ...
        if exitIcon.availableSizes():
            menu.addAction(exitIcon, "Quit GFP ActivityWatch Admin", lambda: exit(self.manager))
        else:
            menu.addAction("Quit GFP ActivityWatch Admin", lambda: exit(self.manager))
