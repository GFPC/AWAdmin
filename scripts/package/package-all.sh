...
function build_zip() {
    ...
    pushd dist;
    filename="gfp_activitywatch_admin-${version}-${platform}-${arch}.zip"
    echo "Name of package will be: $filename"

    if [[ $platform == "windows"* ]]; then
        7z a $filename gfp_activitywatch_admin;
    else
        zip -r $filename gfp_activitywatch_admin;
    fi
    popd;
    echo "Zip built!"
}
...
function build_setup() {
    filename="gfp_activitywatch_admin-${version}-${platform}-${arch}-setup.exe"
    echo "Name of package will be: $filename"
    ...
    mv dist/gfp_activitywatch_admin-setup.exe dist/$filename
    echo "Setup built!"
}
