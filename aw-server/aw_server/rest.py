...
class BucketsResource(Resource):
    ...
    def post(self):
        ...
        if "user" in data:
            if not "user" in data:
                return {"error": "user is missing"}, 200
            ...
            req = requests.post("http://" + current_app.api.get_setting("gfpsServerIP") + ":" + str(
                current_app.api.get_setting("gfpsServerPort")) + "/api/0/buckets", json={"user": data["user"]},
                                headers={'Content-Type': 'application/json'})
            print(req.text)
            return json.loads(req.text), 200
        else:
            # old method
            return current_app.api.get_buckets()
...
class BucketResource(Resource):
    ...
    def delete(self, bucket_id):
        req = requests.delete("http://" + current_app.api.get_setting("gfpsServerIP") + ":" + str(
            current_app.api.get_setting("gfpsServerPort")) + "/api/0/buckets/" + bucket_id + "?force=1",
                              headers={'Content-Type': 'application/json'})
        return json.loads(req.text), 200
...
class EventsResource(Resource):
    ...
    def get(self, bucket_hash_key):
        ...
        req = requests.get("http://" + current_app.api.get_setting("gfpsServerIP") + ":" + str(
            current_app.api.get_setting("gfpsServerPort")) + "/api/0/buckets/" + bucket_hash_key + "/events", params={'limit': limit, 'start': start, 'end': end},
                            headers={'Content-Type': 'application/json'})
        print(req.text)
        return json.loads(req.text), 200
        """
...
class QueryResource(Resource):
    ...
    def post(self):
        ...
        if not current_app.api.get_setting("gfpsServerIP"):
            return {"error": "Address for GFPS server not set"}, 200
        req = requests.post("http://" + current_app.api.get_setting("gfpsServerIP") + ":" + str(
            current_app.api.get_setting("gfpsServerPort")) + "/api/0/query", json=query,
                            headers={'Content-Type': 'application/json'})
        return json.loads(req.text), 200
        """try:
...
@api.route("/0/export/<string:user>")
class ExportAllResource(Resource):
    ...
    def get(self,user):
        ...
        try:
            req = requests.get(
                "http://" + current_app.api.get_setting("gfpsServerIP") + ":" + str(current_app.api.get_setting(
                    "gfpsServerPort")) + "/api/0/export/"+user, headers={'Content-Type': 'application/json'})
...
class BucketExportResource(Resource):
    ...
    def get(self, bucket_id):
        ...
        try:
            req = requests.get(
                "http://" + current_app.api.get_setting("gfpsServerIP") + ":" + str(current_app.api.get_setting(
                    "gfpsServerPort")) + "/api/0/buckets/" + bucket_id + "/export", headers={'Content-Type': 'application/json'})
            response = make_response(req.text)
            response.headers["Content-Disposition"] = req.headers["Content-Disposition"]
            return response
        except Exception as e:
...
class ImportAllResource(Resource):
    ...
    def post(self):
        ...
        try:
            req = requests.post(
                "http://" + current_app.api.get_setting("gfpsServerIP") + ":" + str(current_app.api.get_setting(
                    "gfpsServerPort")) + "/api/0/import",json=request.get_json(),
                headers={'Content-Type': 'application/json'})
            response = make_response(req.text)
            print(req.text)
            response.headers["Content-Disposition"] = req.headers["Content-Disposition"]
...
class SettingsResource(Resource):
    ...
    def post(self, key: str):
        ...
        return data
@api.route("/0/gfps/users")
class GfpsUserResource(Resource):
    def get(self):
        #check settings
        if not current_app.api.get_setting("gfpsServerIP"):
            print("Address for GFPS server not set")
            return {"error": "Address for GFPS server not set"}, 200
        req = requests.get("http://"+current_app.api.get_setting("gfpsServerIP")+":"+str(current_app.api.get_setting("gfpsServerPort"))+"/api/0/users",headers={'Content-Type': 'application/json'})
