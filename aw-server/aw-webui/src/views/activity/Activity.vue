...
export default {
  ...
  mounted: async function () {
    this.username = this.usersStore.users[0]?.username
  ...
  methods: {
    ...
    setDate: function (date, periodLength) {
      ...
      const path = `/activity/${periodLength}/${new_date}/${this.subview}/${this.currentViewId}`;
    ...
    setUsername: async function (username) {
      ...
      this.usersStore.currentUserID = this.usersStore.users.find(u => u.username == this.username).id;
      useBucketsStore().loaded = false;
      await useBucketsStore().ensureLoaded();
      this.activityStore.loaded = false;
      this.activityStore.resetState();
      await this.refresh(true);
      //this.setDate(this._date, this.periodLength);
    },
