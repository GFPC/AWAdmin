...
export const useBucketsStore = defineStore('buckets', {
  ...
  actions: {
    async ensureLoaded(): Promise<void> {
      if (!this.loaded) {
        await this.loadBuckets(this.usersStore.currentUserID !== -1 ? this.usersStore.currentUserID.toString() : undefined);
      }
    },
    async reloadBuckets(): Promise<void> {
      await this.loadBuckets(this.usersStore.currentUserID !== -1 ? this.usersStore.currentUserID.toString() : undefined);
    },

    async loadBuckets(user?: string): Promise<void> {
      this.buckets = [];
      if(this.usersStore.users.length === 0) {
        console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!++++++++++++++++++++==No users found, not loading buckets")
        return
      }
      if (user=='-1' || !user) {
        user = this.usersStore.users[0].id;
      }

      const buckets = await getClient().getBuckets(user);

      await this.update_buckets(buckets);
      this.loaded = true;
    },
    ...
    async getBucketWithEvents({
      ...
      bucket.events = await getClient().getEvents(bucket.hash_key, {
        start,
        end,
        limit: limit || -1,
      });
