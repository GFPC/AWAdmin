  ...
  methods: {
    ...
    update() {
      ...
      let groups = _.map(buckets, bucket => {
        ...
        console.log(useUsersStore().users, bucket.user);
        return { id: bucket.hash_key, content: this.showRowLabels ? useUsersStore().users.find((u: any) => u.id == bucket.user)?.username + ' -> ' + bucket.type : '' };
      });
