...
SUBMODULES := aw-core aw-qt aw-server
...
package:
	rm -rf dist
	mkdir -p dist/gfp_activitywatch_admin
	for dir in $(PACKAGEABLES); do \
		make --directory=$$dir package; \
		cp -r $$dir/dist/$$dir dist/gfp_activitywatch_admin; \
	done
# Move aw-qt to the root of the dist folder
	mv dist/gfp_activitywatch_admin/aw-qt aw-qt-tmp
	mv aw-qt-tmp/* dist/gfp_activitywatch_admin
	rmdir aw-qt-tmp
# Remove problem-causing binaries
	rm -f dist/gfp_activitywatch_admin/libdrm.so.2       # see: https://github.com/ActivityWatch/activitywatch/issues/161
	rm -f dist/gfp_activitywatch_admin/libharfbuzz.so.0  # see: https://github.com/ActivityWatch/activitywatch/issues/660#issuecomment-959889230
...
#   aw-qt: symbol lookup error: /opt/activitywatch/libQt5XcbQpa.so.5: undefined symbol: FT_Get_Font_Format
	rm -f dist/gfp_activitywatch_admin/libfontconfig.so.1
	rm -f dist/gfp_activitywatch_admin/libfreetype.so.6
# Remove unnecessary files
	rm -rf dist/gfp_activitywatch_admin/pytz
